---
name: orient
description: Manual-only orientation workflow. Use only when the user explicitly invokes $orient and asks Codex to orient around a topic, feature, bug, repo area, or codebase context before doing work. This skill gathers surrounding codebase context, reads applicable agent instructions, attempts to title the Codex thread from .agent.env and the prompt, identifies likely ports and simulator/device targets, reports what it learned, and then stops without starting implementation or other work.
---
...
The only allowed state-changing action during orientation is attempting to set the current Codex thread title as described below.
...
2. Attempt to set the current Codex thread title:
   - Read `.agent.env` from the current repository root if present.
   - Extract the sequence number from the `IOS_DEVICE` value. For example, `IOS_DEVICE=Agent-2-iPhone` means the number is `2`.
   - Summarize the user's `$orient` prompt into a concise title of 2-7 words. Remove the `$orient` invocation token and avoid generic titles such as `Orient`.
   - Attempt to set the Codex thread title to `$number - $title`, such as `2 - Investigate Login Flow`.
   - If the thread-title tool is not already available, search for the relevant Codex thread title tool first.
   - If `.agent.env` is missing, `IOS_DEVICE` is missing, the number cannot be parsed, or no title tool is available, do not guess. Continue orientation and report why the title was not set.
3. Read the active agent instructions:
...
4. Identify the repository and project shape:
...
6. Determine ports without starting services:
...
7. Determine simulator/device targets without launching them:
...
8. Stop with a concise orientation report.
...
- `Thread Title`: The attempted title and whether setting it succeeded, failed, or was skipped.
